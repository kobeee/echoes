...
**可点击元素**:
1. 雷达区域: 显示附近Echo的位置点
2. "发现附近的回响"卡片: 点击后切换到Pickup流程
3. 右上角红色感叹号: SOS入口（进入BlackBoxView）
...
**可点击元素**:
1. "切换目标": 循环切换 nearbyEchoes 中的下一个
2. "解锁内容": 触发 `store.tapEcho(echo)`，根据Echo类型弹出不同Sheet
...
点击"解锁内容"
    ↓
store.tapEcho(echo)
    ├── echo.isTimeLocked == true
    │       ↓
    │   TimeLockLockedView (Sheet)
    │   显示倒计时，无法解锁
    │
...
**可点击元素**:
1. "Recovery Key": 打开 `RecoveryKeyView` (Sheet)
2. "Pro": 打开 `ProSubscriptionView` (Sheet)
3. 各种开关: Toggle设置项

### 9.4 全屏页面

#### BlackBoxView (SOS紧急模式)
```
入口: 首页右上角红色感叹号
...
│         未授权        部分授权       已授权                      │
│              ↓             ↓             ↓                      │
│      PermissionFlowView ──→ MainShellView                       │
│                                                    │             │
│                    ┌──────────┬──────────┬───────┼───────┐      │
│                    ↓          ↓          ↓       ↓       ↓      │
│                  首页       地图       埋藏     足迹    设置    │
...
```swift
@Published var phase: AppPhase          // .launch / .permissions / .main
@Published var selectedTab: AppTab      // 当前Tab
@Published var modalRoute: ModalRoute?  // Sheet路由
@Published var fullScreenRoute: ...     // 全屏路由
@Published var echoes: [EchoItem]       // Echo列表
@Published var nearbyEchoes: [EchoItem] // 附近Echo (≤500m)
```

**Modal路由**:
```swift
enum ModalRoute: Identifiable {
    case dropSuccess(title: String)
    case echoContent(id: UUID)
    case passcode(id: UUID)
    case timeLock(id: UUID)
    case sosComplete(key: String)
    case recoveryKey
